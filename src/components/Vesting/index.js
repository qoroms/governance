...
const Vesting = () => {
    ...
    const getHoldings = async () => {
        ...
        if (info) {
            ...
            const zoraInLockup = Number(info.vestedTokens) -
    ...
    const handleVerifyAndDeposit = async () => {
        ...
        if (
            ...
            message = "Beneficiary already have locked ZORA in vesting contract !!";
        }
    ...
    const checkTokenApproval = async () => {
        const tokenApproved = await window.zoraContract
            .methods.allowance(
                details.beneficiary,
        ...
        if (Number(tokenApproved) < Number(details.amount)) {
            window.zoraContract.methods.approve
                (
                    config.vestingAddress,
                    precision.add(10000),
                )
                                ...
                                <div className="holdings-panel__holdings">
                                    <div className="holdings-panel__holdings__row">
                                        ...
                                        <p className="holdings-panel__holdings__row__value">
                                            <span>{precision.remove(Number(state.zoraInLockup))} </span>
                                        </p>
                                    </div>

                                    {Number(state.zoraInLockup) > 0 ?
                                        <div>
                                            <div className="holdings-panel__holdings__row">
                            ...
                            <div className="governance-panel">
                                <div>
                                    ...
                                    {!metamaskError ?
                                        <div className="p-4">
                                            ...
                                            <Row style={{ marginBottom: "15px" }}>
                                                ...
                                                <Col sm={8}>
                                                    <input
                                                        className="vote-button mb-3"
                                                        placeholder="Number of ZORA to Lock"
                                                        type="number"
                                                        step="0"
                                                        min="0"
                                                        onChange={(e) => setDetails({
                                                    ...
                                                    <input
                                                        ...
                                                        value={
                                                            Number(details.vestingPeriod) ?
                                                                (Number(details.amount) / Number(details.vestingPeriod)) + " ZORA"
                                                                : ""
                                                        }
                                                    />
                                                </Col>
                                            ...
                                            {Number(state.zoraInLockup) > 0 ?
                                                <div className="warning-info">
                                                    You already have locked ZORA in Vesting contract.
                                                </div>
                                                : null
                                            }
                                        </div>
                                        :
                                        <div
                                            style={{ marginTop: "20%", paddingBottom: "18%" }}
                                        >
